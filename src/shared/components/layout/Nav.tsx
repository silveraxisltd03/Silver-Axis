...
export function Nav() {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      nav.style.setProperty('-webkit-backdrop-filter', solid ? 'blur(14px)' : 'none');
      ...
      links.forEach((l) => ((l as HTMLElement).style.color = solid ? '#6b7488' : '#cfd6e6'));
      if (cta) {
    ...
    <>
      <nav id="nav" className={menuOpen ? 'nav--open' : ''} style={css('position:fixed;top:0;left:0;right:0;z-index:100;display:flex;align-items:center;justify-content:space-between;padding:20px 40px;transition:background .4s,backdrop-filter .4s,border-color .4s,padding .4s;border-bottom:1px solid transparent;')}>
        ...
        <div className="nav-links" style={css('display:flex;align-items:center;gap:36px;')}>
          <Link to={ROUTES.services} className="navlink" style={css('font-size:15px;color:#cfd6e6;')}>Services</Link>
          <Link to={ROUTES.projects} className="navlink" style={css('font-size:15px;color:#cfd6e6;')}>Projects</Link>
          <Link to={ROUTES.contact} className="navlink" style={css('font-size:15px;color:#cfd6e6;')}>Contact</Link>
        </div>
        <Link to={ROUTES.contact} id="navCta" className="nav-cta btnW" style={css('background:#fff;color:#070b16;font-size:14.5px;font-weight:600;padding:11px 24px;border-radius:999px;white-space:nowrap;transition:background .4s,color .4s,transform .4s cubic-bezier(.16,1,.3,1),box-shadow .4s;')}>Book a free audit</Link>
        <button
          id="navToggle"
          type="button"
      ...
      <div className={`nav-drawer${menuOpen ? ' nav-drawer--open' : ''}`} aria-hidden={!menuOpen}>
        ...
        <Link to={ROUTES.contact} id="navMobileCta" className="btnW nav-drawer-cta" onClick={closeMenu}>Book a free audit</Link>
      </div>
    </>
  );
}
