'use client';

import { useState } from 'react';
import Link from 'next/link';
import { CaseStudyCard } from '@/shared/components/ui/CaseStudyCard';
import { Reveal } from '@/shared/components/ui/Reveal';
import { TechPill } from '@/shared/components/ui/TechPill';
import { getCaseStudyBySlug, getRelatedCaseStudies } from '@/shared/content/case-studies';
import { projectDetailPath, ROUTES } from '@/shared/constants/routes';
import type { CaseStudy } from '@/shared/types';

function IconX({ className }: { className?: string }) {
  return (
    <svg className={className} width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" aria-hidden="true">
      <path d="M18 6 6 18M6 6l12 12" />
    </svg>
  );
}

function IconCheck({ className }: { className?: string }) {
  return (
    <svg className={className} width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" aria-hidden="true">
      <path d="M20 6 9 17l-5-5" />
    </svg>
  );
}

function IconExpand() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
      <path d="M15 3h6v6M9 21H3v-6M21 3l-7 7M3 21l7-7" />
    </svg>
  );
}

interface CaseStudyDetailPageProps {
  slug: string;
}

export function CaseStudyDetailPage({ slug }: CaseStudyDetailPageProps) {
  const cs = getCaseStudyBySlug(slug);
  const [lightbox, setLightbox] = useState<{ src: string; caption: string } | null>(null);

  if (!cs) {
    return (
      <div className="section-x container cs-hero">
        <p>Case study not found.</p>
        <Link href={ROUTES.projects} className="cs-back-link">
          ← All projects
        </Link>
      </div>
    );
  }

  const related = getRelatedCaseStudies(slug);

  return (
    <div className="cs-detail-page">
      <header className="cs-hero section-x container">
        <Reveal>
          <Link href={ROUTES.projects} className="cs-back-link">
            ← All projects
          </Link>
          <span className="cs-tag">{cs.tag}</span>
          <h1 className="cs-hero__title">{cs.title}</h1>
          <p className="cs-hero__desc">{cs.desc}</p>
          {cs.liveUrl && (
            <a href={cs.liveUrl} target="_blank" rel="noopener noreferrer" className="cs-live-btn">
              View Live ↗
            </a>
          )}
        </Reveal>
      </header>

      {cs.thumbnail && (
        <Reveal>
          <div className="cs-thumbnail section-x container">
            <div className="cs-thumbnail__frame">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={cs.thumbnail}
                alt={`${cs.title}, project screenshot`}
                className="cs-thumbnail__img"
                loading="eager"
              />
            </div>
          </div>
        </Reveal>
      )}

      <div className="cs-metrics-strip section-x container">
        <div className="cs-metric-card">
          <span className="cs-metric-card__value cs-metric-card__value--dark">{cs.m1v}</span>
          <span className="cs-metric-card__label cs-metric-card__label--dark">{cs.m1l}</span>
        </div>
        <div className="cs-metric-card">
          <span className="cs-metric-card__value cs-metric-card__value--dark">{cs.m2v}</span>
          <span className="cs-metric-card__label cs-metric-card__label--dark">{cs.m2l}</span>
        </div>
        {cs.challenge && (
          <div className="cs-metric-card cs-metric-card--wide">
            <span className="cs-metric-card__label cs-metric-card__label--dark">Challenge</span>
            <p className="cs-metric-card__blurb">{cs.challenge}</p>
          </div>
        )}
      </div>

      {cs.testimonial && (
        <Reveal>
          <div className="cs-testimonial section-x container">
            <blockquote className="cs-testimonial__block">
              <span className="cs-testimonial__eyebrow">Client feedback</span>
              <p className="cs-testimonial__quote">&ldquo;{cs.testimonial.quote}&rdquo;</p>
              {(cs.testimonial.author || cs.testimonial.role) && (
                <footer className="cs-testimonial__footer">
                  {cs.testimonial.author && <span className="cs-testimonial__author">{cs.testimonial.author}</span>}
                  {cs.testimonial.role && <span className="cs-testimonial__role">{cs.testimonial.role}</span>}
                </footer>
              )}
            </blockquote>
          </div>
        </Reveal>
      )}

      {(cs.problemPoints || cs.solutionPoints) && (
        <Reveal>
          <div className="cs-ba section-x container">
            <div className="cs-ba__header">
              <h2 className="cs-ba__heading">The transformation</h2>
              <p className="cs-ba__sub">What the client was dealing with, and what we replaced it with.</p>
            </div>
            <div className="cs-ba__panels">
              {cs.problemPoints && (
                <div className="cs-ba__panel cs-ba__panel--before">
                  <div className="cs-ba__panel-label">
                    <span className="cs-ba__badge cs-ba__badge--before">Before</span>
                  </div>
                  <ul className="cs-ba__list">
                    {cs.problemPoints.map((p) => (
                      <li key={p} className="cs-ba__item cs-ba__item--before">
                        <IconX className="cs-ba__icon cs-ba__icon--before" />
                        <span>{p}</span>
                      </li>
                    ))}
                  </ul>
                </div>
              )}

              <div className="cs-ba__arrow" aria-hidden="true">
                →
              </div>

              {cs.solutionPoints && (
                <div className="cs-ba__panel cs-ba__panel--after">
                  <div className="cs-ba__panel-label">
                    <span className="cs-ba__badge cs-ba__badge--after">After</span>
                  </div>
                  <ul className="cs-ba__list">
                    {cs.solutionPoints.map((p) => (
                      <li key={p} className="cs-ba__item cs-ba__item--after">
                        <IconCheck className="cs-ba__icon cs-ba__icon--after" />
                        <span>{p}</span>
                      </li>
                    ))}
                  </ul>
                </div>
              )}
            </div>
          </div>
        </Reveal>
      )}

      {(cs.problemPoints || cs.solutionPoints) && (
        <Reveal>
          <div className="cs-inline-cta section-x container">
            <Link href={ROUTES.contact} className="cs-inline-cta__btn">
              Got a similar problem? Let&apos;s talk →
            </Link>
          </div>
        </Reveal>
      )}

      {cs.deliverySteps && cs.deliverySteps.length > 0 && (
        <Reveal>
          <div className="cs-delivery section-x container">
            <h2 className="cs-delivery__heading">How we built it</h2>
            <ol className="cs-delivery__steps">
              {cs.deliverySteps.map((step, i) => (
                <li key={step} className="cs-delivery__step">
                  <div className="cs-delivery__step-n">{i + 1}</div>
                  <p className="cs-delivery__step-text">{step}</p>
                </li>
              ))}
            </ol>
          </div>
        </Reveal>
      )}

      {cs.tools && cs.tools.length > 0 && (
        <Reveal>
          <div className="cs-tools-section section-x container">
            <h2 className="cs-tools-section__heading">Technologies used</h2>
            <div className="cs-tools-wrap">
              {cs.tools.map((tool) => (
                <TechPill key={tool} name={tool} size="md" />
              ))}
            </div>
          </div>
        </Reveal>
      )}

      {cs.screenshots && cs.screenshots.length > 0 && (
        <Reveal>
          <div className="cs-gallery section-x container">
            <h2 className="cs-gallery__heading">Project screenshots</h2>
            <div className="cs-gallery__grid">
              {cs.screenshots.map((shot) => (
                <button
                  key={shot.src}
                  type="button"
                  className="cs-gallery__item"
                  onClick={() => setLightbox(shot)}
                  aria-label={`View screenshot: ${shot.caption}`}
                >
                  <div className="cs-gallery__img-wrap">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={shot.src} alt={shot.caption} className="cs-gallery__img" loading="lazy" />
                    <span className="cs-gallery__overlay" aria-hidden="true">
                      <IconExpand />
                    </span>
                  </div>
                  <p className="cs-gallery__caption">{shot.caption}</p>
                </button>
              ))}
            </div>
          </div>
        </Reveal>
      )}

      {lightbox && (
        <div
          className="cs-lightbox"
          onClick={() => setLightbox(null)}
          onKeyDown={(e) => e.key === 'Escape' && setLightbox(null)}
          role="dialog"
          aria-modal="true"
          aria-label={lightbox.caption}
        >
          <button type="button" className="cs-lightbox__close" onClick={() => setLightbox(null)} aria-label="Close">
            <IconX />
          </button>
          <div className="cs-lightbox__inner" onClick={(e) => e.stopPropagation()}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={lightbox.src} alt={lightbox.caption} className="cs-lightbox__img" />
            <p className="cs-lightbox__caption">{lightbox.caption}</p>
          </div>
        </div>
      )}

      {related.length > 0 && (
        <section className="cs-related section-x container">
          <Reveal>
            <h2 className="cs-related__title">More in this space</h2>
          </Reveal>
          <div className="projects-grid projects-grid--related">
            {related.map((r: CaseStudy) => (
              <Reveal key={r.slug}>
                <CaseStudyCard caseStudy={r} to={projectDetailPath(r.slug)} />
              </Reveal>
            ))}
          </div>
        </section>
      )}

      <section className="cs-banner section-x">
        <div className="container cs-banner__inner">
          <h2 className="cs-banner__title">Want results like this?</h2>
          <p className="cs-banner__body">
            Start with a free audit—we will map what to automate, what to rebuild, and what to leave alone.
          </p>
          <Link href={ROUTES.contact} className="cs-banner__btn">
            Book a free audit
          </Link>
        </div>
      </section>
    </div>
  );
}
